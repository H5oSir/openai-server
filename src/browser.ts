...
export default async function getBrowser(headless?: boolean = true): (Browser | Page)[] {
  ...
  const browser: Browser = await puppeteer.launch({
    headless,
    // devtools: true,
    defaultViewport: { width: 750, height: 420 },
    args: puppeteerArgs,
    ignoreDefaultArgs: [
      '--disable-extensions',
      '--enable-automation',
...
export async function html2jpg(htmlText: string): Promise<string> {
  ...
  const html = path.join(path.resolve(), `tmp/${genUid()}.html`)
  ...
  await page.reload()
  const jpg = path.join(path.resolve(), `tmp/${genUid()}.jpg`)
